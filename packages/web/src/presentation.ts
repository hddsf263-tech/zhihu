...
export function markdownPlan(map: ExperienceMap, route: Route, completed: string[]): string {
  ...
  const lines = [`# ${escape(route.title)}｜行动清单`, '', `问题：${escape(map.query ?? map.questionUrl ?? '知乎问题')}`,
  ...
  for (const stage of route.stages) {
    lines.push(`## ${escape(stage.title)} · ${escape(stage.suggestedWeeks)}`, '');
    for (const task of stage.tasks) {
      lines.push(`- [${completed.includes(task.taskId) ? 'x' : ' '}] ${escape(task.action)}`, `  完成判据：${escape(task.doneWhen)}`);
