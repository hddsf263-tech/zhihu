...
const places = [
  { id: 'portfolio', title: '作品村', subtitle: '先做可展示作品', tone: 'brick', x: '15%', y: '66%', icon: '⌂', status: '可进入' },
  { id: 'feedback', title: '投递码头', subtitle: '尽早获得反馈', tone: 'forest', x: '70%', y: '43%', icon: '⚑', status: '可进入' },
  { id: 'skills', title: '技能森林', subtitle: '补齐工具方法', tone: 'gold', x: '72%', y: '74%', icon: '✦', status: '可进入' },
];

function Guide() {
  const [wave, setWave] = useState(false);
  const [open, setOpen] = useState(false);
  return <button className={`guide ${open ? 'open' : ''} ${wave ? 'hovered' : ''}`} onClick={() => setOpen((v) => !v)} onMouseEnter={() => setWave(true)} onMouseLeave={() => setWave(false)} onFocus={() => setWave(true)} onBlur={() => setWave(false)} aria-expanded={open} aria-label="刘看山：打开地图提示">
    <span className="guide-aura" aria-hidden="true" />
    <img src="/assets/liukanshan-guide.gif" alt="刘看山向导" />
...
function MapNode({ place }: { place: typeof places[number] }) {
  return <Link to={`/jobs/job_replay_pm_intern?route=${place.id}`} className={`map-node ${place.tone}`} style={{ left: place.x, top: place.y }} aria-label={`${place.title}：${place.subtitle}`}>
    ...
    <span className="node-plaque"><span className="sign-title">{place.title}</span><span className="sign-subtitle">{place.subtitle}</span><small>{place.status} →</small></span>
  </Link>;
}
...
function Home() {
  ...
  return <main className="world-page">
    ...
    <section className="map-shell" aria-label="知乎经验地图首页">
      ...
      <Guide />
      <button className={`mystery-zone ${discovered ? 'discovered' : ''}`} onClick={() => setDiscovered(true)} aria-label="发现未知区域"><span className="mystery-spark">{discovered ? '✦' : '?'}</span><strong>{discovered ? '新地点发现！' : '未知区域'}</strong><small>{discovered ? '隐藏补给站 · 已解锁' : '点击探索'}</small></button>
      ...
      <div className="map-legend" aria-label="地图图例"><span><i className="legend-dot ready" />可进入</span><span><i className="legend-dot found" />已发现</span><span><i className="legend-dot mystery" />待探索</span></div>
    </section>
    <section className="world-footer"><div><strong>地图状态</strong><span>3 个地点 · 2 个阶段 · {discovered ? '2 枚已发现证据' : '1 枚待核对证据'}</span></div><div><strong>探索提示</strong><span>Hover 地点查看说明，点击建筑进入路线</span></div></section>
  </main>;
}
